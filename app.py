...
def sync_all_existing_orders_to_google_sheet():
    """Repair/backfill ALL MongoDB purchase history, not only flagged-unsynced rows."""
    if not get_google_sheet_url():
        return 0, 0
    ...
    def flush_batch(items):
        ...
        # IMPORTANT: items also contains the local MongoDB ObjectId under
        # _mongo_id_obj for marking records synced. ObjectId is NOT JSON
        # serializable, so never send that field to Apps Script.
        sheet_items = [
        ...
        result = _post_google_sheet(payload, retries=4)
        if not result:
            failed += len(items)
            return
        ...
        # Any order not explicitly confirmed is treated as failed and can be retried later.
    ...
    try:
        # IMPORTANT: scan ALL active_orders so a bad/stale google_sheet_synced flag
        # cannot permanently hide a missing Sheet row.
        cursor = orders_col.find({}).sort("_id", 1)
        for row in cursor:
