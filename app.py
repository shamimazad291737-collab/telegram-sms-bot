    ...
    try:
        ...
        if "message" in update:
                ...
                if isinstance(state_data, str) and state_data == "BUY_MULTI_QTY":
                    try:
                        ...
                        if bal + 1e-12 < total_cost:
                            send_message(chat_id, f"❌ <b>পর্যাপ্ত ব্যালেন্স নেই!</b>\n{qty} টি নম্বর কিনতে ${format_number_price(total_cost)} USD লাগবে। আপনার ব্যালেন্স: ${bal:.2f} USD।")
                            delete_user_state(user_id)
                            return
                        ...
                        for _ in range(qty):
                            ...
                            if phone:
                                ...
                                p_date = order_doc.get("purchase_date", p_date) if order_doc else p_date
                                log_purchase_to_google_sheet(user_id, username, phone, link, p_date, order_id=order_id)
                        ...
                        res_msg += f"\n💰 <b>মোট ফি কাটা হয়েছে:</b> ${format_number_price(current_price * len(purchased_list))} USD\n👉 যেকোনো একটির ওটিপি পেতে <b>Check OTP</b> বাটনে চাপ দিন।"
            ...
            if data == "admin_toggle_bot_off" and is_admin:
                ...
                # Send the purchased number first. If Telegram rejects the message,
                # put the stock item back and do not charge the user.
                delivery_result = send_message(chat_id, res_text, reply_markup=markup)
                if not delivery_result.get("ok"):
                    add_stock_item(phone, link)
                    edit_message(chat_id, message_id, "⚠️ <b>নম্বর পাঠাতে সমস্যা হয়েছে।</b> আপনার ব্যালেন্স কাটা হয়নি। কিছুক্ষণ পর আবার চেষ্টা করুন।")
                    return

                # Charge only after successful delivery to the user.
                deduct_balance(user_id, current_price)
                order_id = save_active_order(user_id, phone, link)

                # Log to Google Sheet using the exact MongoDB order id.
                order_doc = orders_col.find_one({"_id": order_id}, {"purchase_date": 1})
                p_date = order_doc.get("purchase_date", datetime.now().strftime("%d-%b-%Y %I:%M %p")) if order_doc else datetime.now().strftime("%d-%b-%Y %I:%M %p")
                log_purchase_to_google_sheet(user_id, username, phone, link, p_date, order_id=order_id)
