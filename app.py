...
ADMIN_ID = (
    int(os.environ.get("ADMIN_ID", "0")) if os.environ.get("ADMIN_ID") else 0
)
...
def add_user(user_id, username):
  users_col.update_one(
      {"user_id": user_id},
      {
          ...
          "$setOnInsert": {"balance": 0.0, "total_recharge": 0.0},
      },
      upsert=True,
  )
...
def get_user_orders_24h(user_id):
  ...
  orders = list(
      orders_col.find(
          {"user_id": user_id, "timestamp": {"$gte": time_24h_ago}}
      ).sort("_id", -1)
  )
  return orders
  ...
  for ch in REQUIRED_CHANNELS:
    ...
    try:
      res = requests.post(
          ...
          json={"chat_id": ch_username, "user_id": user_id},
          timeout=5,
      ).json()
      if res.get("ok"):
...
def send_force_join_msg(chat_id):
  ...
  msg_text = (
      "<b>⚠️ বটে কাজ করার জন্য নিচের চ্যানেলগুলোতে জয়েন করা"
      " বাধ্যতামূলক:</b>\n\nসবগুলো চ্যানেলে জয়েন করে <b>Verify</b> বাটনে চাপ"
      " দিন।"
  )
...
def send_message(chat_id, text, reply_markup=None):
  ...
  try:
    return requests.post(
        BASE_URL + "sendMessage", json=payload, timeout=10
    ).json()
...
# Safe Execution Wrapper Error Fix
def safe_execution_wrapper(update):
  try:
...
def handle_update(update):
  if "message" in update:
    ...
    if not is_bot_active() and not is_admin:
      send_message(
          chat_id,
          "⚠️ <b>এডমিন বর্তমানে বট সার্ভিস অফ রেখেছেন।</b>\nপরবর্তী"
    ...
    if user_id in user_states:
      ...
      if (
          ...
          and state_data.get("step") == "WAITING_AMOUNT"
      ):
        ...
        try:
          ...
          if method == "BKASH":
            msg_text = (
                f"💖 <b>bKash Send Money</b>\n\n💵 <b>ডিপোজিট:</b> ৳{amount:.2f}"
                f" BDT\n📱 <b>bKash:</b> <code>{BKASH_NUMBER}</code>\n\nটাকা"
                " পাঠানোর পর আপনার <b>TrxID</b> লিখে পাঠান:"
            )
          elif method == "NAGAD":
            msg_text = (
            ...
            msg_text = (
                ...
                " পাঠানোর পর আপনার <b>Order ID / TrxID</b> লিখে পাঠান:"
            )
          ...
          isinstance(state_data, dict)
          and state_data.get("step") == "WAITING_TRX"
      ):
        method = state_data["method"]
        ...
        send_message(
            chat_id,
            "📸 <b>পেমেন্টের একটি স্পষ্ট স্ক্রিনশট (Photo) পাঠান:</b>",
        ...
        return
          ...
          isinstance(state_data, dict)
          and state_data.get("step") == "WAITING_SCREENSHOT"
      ):
        if "photo" in msg:
          ...
          if method == "BINANCE":
            ...
            amount_info = (
                f"৳{amount:.2f} BDT (Estimated: ${converted_usd:.2f} USD @"
                f" {bdt_per_usd} BDT/$)"
            )
          ...
          admin_markup = {
              "inline_keyboard": [
                  [
                      {
                          "text": f"✅ Auto Approve (${converted_usd:.2f})",
                          "callback_data": (
                              f"appusd_{user_id}_{usd_str_clean}"
                          ),
                          "style": "success",
                      },
                      {
                          "text": "✏️ Custom Amount",
                          "callback_data": f"dep_app_{user_id}",
                          "style": "primary",
                      },
                  ],
                  [{
                      "text": "❌ Reject Request",
                      "callback_data": f"dep_rej_{user_id}",
                      "style": "danger",
                  }],
              ]
          }
          admin_caption = (
              ...
              "যাচাই করে ডলার যোগ করতে বাটন চাপুন:"
          )

          send_photo_to_admin(
              ADMIN_ID, photo_file_id, admin_caption, reply_markup=admin_markup
          )
          send_message(
              chat_id,
              "✅ <b>আপনার তথ্য ও স্ক্রিনশট এডমিনের কাছে পাঠানো হয়েছে!</b>",
              reply_markup=get_main_keyboard(is_admin),
          )
          del user_states[user_id]
          return
          ...
          send_message(
              chat_id,
              "❌ <b>অনুগ্রহ করে পেমেন্টের একটি ফটো/স্ক্রিনশট পাঠান।</b>",
              reply_markup=get_back_keyboard(),
          )
          return
        ...
        send_message(
            chat_id,
            f"✅ <b>সফলভাবে {added_count} জন ইউজার সিস্টেমে এড/আপডেট করা"
            " হয়েছে!</b>",
            reply_markup=get_main_keyboard(is_admin),
        )
        del user_states[user_id]
        ...
        try:
          ...
          send_message(
              chat_id,
              f"✅ <b>নতুন ডলার রেট ৳{new_rate} BDT সেট করা হয়েছে!</b>",
              reply_markup=get_main_keyboard(is_admin),
          )
        except ValueError:
          send_message(
              chat_id, "❌ <b>সঠিক সংখ্যা লিখুন (যেমন: 128 বা 130)।</b>"
          )
        del user_states[user_id]
        return
        ...
        try:
          ...
          send_message(
              chat_id,
              f"✅ <b>নম্বরের নতুন দাম ${new_p:.2f} USD সেট করা হয়েছে!</b>",
              reply_markup=get_main_keyboard(is_admin),
          )
        except ValueError:
          send_message(
        ...
        if "document" in msg:
          ...
          if not (file_name.endswith(".txt") or file_name.endswith(".csv")):
            send_message(
                chat_id, "❌ <b>শুধুমাত্র .txt বা .csv ফাইল পাঠান।</b>"
            )
            return
          ...
          if file_info.get("ok"):
            ...
            send_message(
                chat_id,
                f"✅ <b>{count} টি নম্বর স্টকে যোগ করা হয়েছে!</b>",
                reply_markup=get_main_keyboard(is_admin),
            )
            del user_states[user_id]
            return
      ...
      elif isinstance(state_data, str) and state_data.startswith(
          "ADMIN_APPROVE_AMOUNT_"
      ):
        ...
        try:
          ...
          send_message(
              chat_id,
              f"✅ <b>User ID {target_user}-কে ${usd_val:.2f} USD যোগ করা"
              " হয়েছে।</b>",
          )
          send_message(
              target_user,
              f"🎉 <b>আপনার অ্যাকাউন্টে ${usd_val:.2f} USD ডিপোজিট যোগ"
              " হয়েছে।</b>",
          )
        except Exception:
        ...
        try:
          ...
          send_message(
              chat_id,
              f"✅ <b>User ID {target_user}-কে ${usd_val:.2f} USD রিফান্ড করা"
              " হয়েছে।</b>",
          )
          send_message(
              target_user,
              f"🎉 <b>এডমিন আপনার অ্যাকাউন্টে ${usd_val:.2f} USD রিফান্ড"
              " করেছেন!</b>",
          )
        except Exception:
          send_message(chat_id, "❌ <b>ভুল ইনপুট!</b>")
    ...
    if text == "/start":
      ...
      send_message(
          chat_id, welcome_text, reply_markup=get_main_keyboard(is_admin)
      )
      ...
      markup = {
          "inline_keyboard": [[{
              "text": (
                  f"🇺🇸 Buy USA WhatsApp Number (${current_price:.2f} USD)"
              ),
              "callback_data": "confirm_buy_usa",
              "style": "success",
          }]]
      }
      send_message(
          chat_id,
          "<b>WhatsApp Service Selected:</b>\n\nমূল্য:"
          f" <b>${current_price:.2f} USD / Number</b>",
          reply_markup=get_back_keyboard(),
      )
      ...
      dep_text = (
          "💳 <b>Deposit Options</b>\n\n<i>নোট:"
          f" ৳{int(bdt_per_usd)} BDT = $1.00 USD কনভার্ট হবে।</i>\n\nআপনার"
          " সুবিধাজনক মেথড বেছে নিন:"
      )
      markup = {
          "inline_keyboard": [
              [{
                  "text": "💖 bKash (BDT)",
                  "callback_data": "dep_bkash",
                  "style": "primary",
              }],
              [{
                  "text": "🟠 Nagad (BDT)",
                  "callback_data": "dep_nagad",
                  "style": "primary",
              }],
              [{
                  "text": "🟡 Binance (Crypto USDT)",
                  "callback_data": "dep_binance",
                  "style": "success",
              }],
          ]
      }
      ...
      send_message(
          chat_id,
          f"<b>যেকোনো সাহায্যে যোগাযোগ করুন:</b>\n👉 @{SUPPORT_USERNAME}",
          reply_markup=get_back_keyboard(),
      )
      ...
      markup = {
          "inline_keyboard": [
              [{
                  "text": f"🔘 Toggle Bot Status ({status_str})",
              ...
              [{
                  "text": "➕ Bulk Add Users (ID/Username)",
                  "callback_data": "admin_bulk_users",
                  "style": "primary",
              }],
              [{
                  "text": "💱 Change BDT Exchange Rate",
                  "callback_data": "admin_set_bdt_rate",
                  "style": "primary",
              }],
              [{
                  "text": "🏷️ Change WhatsApp Price",
              ...
              [{
                  ...
                  "callback_data": "admin_delete_stock_confirm",
                  "style": "danger",
              }],
          ]
      }
    ...
    if data == "verify_force_join":
      if check_force_join(user_id):
        edit_message(
            chat_id,
            message_id,
            "✅ <b>ভেরিফিকেশন সফল হয়েছে!</b>\nএখন আপনি বটটি ব্যবহার করতে"
            " পারেন।",
        )
        send_message(
            chat_id, "মূল মেনু:", reply_markup=get_main_keyboard(is_admin)
        )
      else:
        send_message(
            chat_id,
            "❌ <b>আপনি এখনও সবগুলো চ্যানেলে জয়েন করেননি!</b>\nঅনুগ্রহ করে সব"
            " চ্যানেলে জয়েন করে আবার ভেরিফাই করুন।",
        )
      return
    ...
    if data == "confirm_buy_usa":
      ...
      markup = {
          "inline_keyboard": [
              [{
                  "text": "🔄 Check OTP",
                  "callback_data": f"chk_otp_{phone}",
                  "style": "primary",
              }],
              [{
                  "text": "🛒 Buy Another Number",
                  "callback_data": "confirm_buy_usa",
                  "style": "success",
              }],
          ]
      ...
      if order:
        ...
        try:
          ...
          api_link = (
              link.replace("/sms/", "/api/sms/") if "/sms/" in link else link
          )
          ...
          if re.match(r"^\d{3,10}$", api_text):
            ...
            for code in matches:
              if (
                  code not in ["111111", "000000", "123456"]
                  and code not in clean_phone
              ):
                otp_code = code
                break
        except Exception as e:
        ...
        if otp_code:
          ...
          send_message(
              chat_id,
              f"📥 <b>আপনার OTP:</b> <code>{otp_code}</code>",
              reply_markup=markup,
          )
      ...
      edit_message(
          chat_id,
          message_id,
          f"⚙️ <b>বটের স্ট্যাটাস পরিবর্তন করা হয়েছে:</b> {new_st}",
      )
      ...
      send_message(
          chat_id,
          "➕ <b>ইউজারদের ID অথবা Username দিন:</b>\n(একসাথে একাধিক দিতে স্পেস"
          " বা নতুন লাইন ব্যবহার করুন। উদাহরণ: <code>12345678 @username2"
          " 87654321</code>)",
          reply_markup=get_back_keyboard(),
      )
      ...
      send_message(
          chat_id,
          "💱 <b>নতুন ১ ডলারের BDT রেট লিখে পাঠান:</b>\n(বর্তমান রেট:"
          f" ৳{get_bdt_rate()} BDT)",
          reply_markup=get_back_keyboard(),
      )
      ...
      for u in users_list:
        ...
        display_title = (
            f"👤 @{u_name} (${u_bal:.2f})"
            if u_name != "NoUsername"
            else f"👤 ID: {u_id} (${u_bal:.2f})"
        )
        buttons.append([{
            "text": display_title,
            "callback_data": f"inspect_u_{u_id}",
            "style": "primary",
        }])
      ...
      send_message(
          chat_id,
          f"👥 <b>বটের ইউজারের তালিকা (মোট: {len(users_list)} জন):</b>",
          reply_markup=markup,
      )
      ...
      user_msg = (
          "👤 <b>USER DETAILS & HISTORY (LAST 24 HOURS)</b>\n\n"
          ...
          f"📊 <b>Total Recharge:</b> ${u_info.get('total_recharge', 0.0):.2f}"
          " USD\n"
      ...
      for idx, ord_item in enumerate(orders, 1):
        ...
        user_msg += (
            f"<b>{idx}.</b> 📱 <code>{ord_item['phone_number']}</code>\n   📅 Date:"
            f" {ord_item['purchase_date']}\n   🔗 Link:"
            f" {ord_item['otp_link']}\n   📌 Status: {status}\n\n"
        )
      ...
      markup = {
          "inline_keyboard": [
              [{
                  "text": "➕ Add / Refund Balance",
                  "callback_data": f"admin_ref_input_{target_u_id}",
                  "style": "success",
              }],
              [{
                  "text": "⬅️ Back",
      ...
      send_message(
          chat_id,
          f"➕ <b>User ID {target_u_id}-এর অ্যাকাউন্টে ডলার ($) অ্যামাউন্ট"
          " লিখুন:</b>",
          reply_markup=get_back_keyboard(),
      )
      ...
      send_message(
          chat_id,
          "💖 <b>bKash Deposit</b>\n\nকত টাকা (BDT) ডিপোজিট করবেন লিখে"
          f" পাঠান:\n<i>(রেট: ৳{int(get_bdt_rate())} BDT = $1.00 USD)</i>",
          reply_markup=get_back_keyboard(),
      )
      ...
      send_message(
          chat_id,
          "🟡 <b>Binance Deposit</b>\n\nকত <b>USDT</b> (USD) ডিপোজিট করবেন লিখে"
          " পাঠান:",
          reply_markup=get_back_keyboard(),
      )
      ...
      send_message(
          chat_id,
          "🏷️ <b>WhatsApp নম্বরের নতুন মূল্য ($ USD) লিখে পাঠান:</b>",
          reply_markup=get_back_keyboard(),
      )
      ...
      markup = {
          "inline_keyboard": [[{
              "text": "✅ Yes, Delete All",
              "callback_data": "admin_delete_stock_execute",
              "style": "danger",
          }]]
      }
      edit_message(
          chat_id,
          message_id,
          "⚠️ <b>সমস্ত স্টক মুছে ফেলতে চান?</b>",
          reply_markup=markup,
      )
      ...
      send_message(
          target_user,
          f"🎉 <b>আপনার ডিপোজিট সফল হয়েছে! ${usd_val:.2f} USD যোগ করা"
          " হয়েছে।</b>",
      )
      ...
      send_message(
          chat_id, f"<b>User ID {target_user}-এর জন্য ডলার অ্যামাউন্ট লিখুন:</b>"
      )

    elif data.startswith("dep_rej_") and is_admin:
      target_user = int(data.replace("dep_rej_", ""))
      send_message(target_user, "❌ <b>ডিপোজিট প্রুফটি সঠিক নয়!</b>")
      send_message(
          chat_id,
          f"❌ <b>User ID {target_user}-এর ডিপোজিট বাতিল করা হয়েছে।</b>",
      )
...
# ১. ব্যাকগ্রাউন্ড থ্রেডে Flask চালু
threading.Thread(target=run_flask, daemon=True).start()

# ২. টেলিগ্রাম Webhook ক্লিয়ার করা
try:
...
# ৩. টেলিগ্রাম পোলিং লুপ (মেইন থ্রেডে)
offset = 0
while True:
  try:
    res = requests.get(
