...
def migrate_money_fields_to_decimal128():
    """Convert legacy float balances to exact MongoDB Decimal128 values."""
    try:
        for u in users_col.find({}, {"_id": 1, "balance": 1, "total_recharge": 1}):
            updates = {}
            for field in ("balance", "total_recharge"):
                value = u.get(field, 0)
                if not hasattr(value, "to_decimal"):
                    updates[field] = Decimal128(to_decimal(value))
            if updates:
                users_col.update_one({"_id": u["_id"]}, {"$set": updates})

        for r in refund_logs_col.find({}, {"_id": 1, "amount": 1}):
            value = r.get("amount", 0)
            if not hasattr(value, "to_decimal"):
                refund_logs_col.update_one(
                    {"_id": r["_id"]},
                    {"$set": {"amount": Decimal128(to_decimal(value))}}
                )
        print("Decimal128 money migration completed.")
...
def _post_google_sheet(payload, retries=3):
    """Reliable Google Apps Script POST with JSON-level success checking."""
    ...
    for attempt in range(retries):
        try:
            ...
            # HTTP 200 alone is NOT considered success; Apps Script must return ok:true.
            if response.ok and isinstance(result, dict) and result.get("ok") is True:
                return result

            last_error = (
                f"HTTP {response.status_code}; "
                f"body={result if result is not None else response.text[:300]}"
            )
        except Exception as e:
            last_error = str(e)
    ...
    print(f"Google Sheet POST failed after {retries} attempts: {last_error}")
    return None


def _mark_order_synced(order_ids):
    """Mark only the exact MongoDB orders confirmed by Apps Script as synced."""
...
def log_purchase_to_google_sheet(user_id, username, phone, otp_link, purchase_date, otp_code=None, order_id=None):
    """Send one purchase to Google Sheets reliably and idempotently."""
    if not get_google_sheet_url():
        return False
    ...
    payload = {
        ...
        "records": [{
            ...
            "user_id": str(user_id),
            "username": str(username or "NoUsername"),
            "phone": str(phone or ""),
            "otp": str(otp_code) if otp_code else "",
            "otp_link": str(otp_link or ""),
            "date": str(purchase_date or "")
        }]
    }

    result = _post_google_sheet(payload, retries=3)
    if not result:
        return False

    synced_ids = set(result.get("synced_order_ids", []))
    if order_id is not None and str(order_id) in synced_ids:
        _mark_order_synced([order_id])
        gid = result.get("gids", {}).get(str(user_id))
        if gid:
    ...
    # Legacy/no-order-id record: Apps Script may report it as already present.
    if order_id is None and result.get("processed", 0) >= 1:
        return True
    return False


def background_log_purchase(user_id, username, phone, link, purchase_date, order_id):
    """Log purchase without delaying Telegram number delivery."""
    try:
        log_purchase_to_google_sheet(
            user_id, username, phone, link, purchase_date, order_id=order_id
        )
    except Exception as e:
        print(f"Background Google Sheet purchase log skipped: {e}")
...
def update_purchase_otp_in_google_sheet(user_id, phone, otp_code, order_id=None):
    """Update the OTP for the exact order when possible."""
    if not get_google_sheet_url() or not otp_code:
        return False
    payload = {
        "action": "otp_update",
        "user_id": str(user_id),
    ...
    return bool(_post_google_sheet(payload, retries=3))


def sync_all_existing_orders_to_google_sheet():
    """Retry only unsynced MongoDB purchase history in small batches.

    Orders already confirmed by Google Apps Script are marked True and are
    skipped on later runs. Failed records therefore get retried without
    ...
    failed = 0
    batch = []
    # Smaller batches reduce Apps Script execution time and request size.
    BATCH_SIZE = 20
    ...
    def flush_batch(items):
        ...
        # Never send the local MongoDB ObjectId object to JSON.
        sheet_items = [
            {k: v for k, v in item.items() if k != "_mongo_id_obj"}
            for item in items
        ]
        payload = {
            "action": "batch_purchase",
        ...
        result = _post_google_sheet(payload, retries=5)
        if not result:
            failed += len(items)
            return

        synced_ids = {str(x) for x in result.get("synced_order_ids", [])}
        order_map = {str(x["_mongo_id"]): x["_mongo_id_obj"] for x in items}
        confirmed = [order_map[x] for x in synced_ids if x in order_map]
        _mark_order_synced(confirmed)
        success += len(confirmed)
        failed += len(items) - len(confirmed)

        gids = result.get("gids", {})
        for uid, gid in gids.items():
            try:
                users_col.update_one(
                    {"user_id": int(uid) if str(uid).lstrip("-").isdigit() else uid},
                    {"$set": {"google_sheet_gid": str(gid)}}
                )
            except Exception:
                pass
    ...
    try:
        # IMPORTANT: only retry records not confirmed by a previous sync.
        cursor = orders_col.find({"google_sheet_synced": {"$ne": True}}).sort("_id", 1)
        for row in cursor:
            user_id = row.get("user_id")
            u = users_col.find_one({"user_id": user_id}, {"username": 1})
            ...
            batch.append({
                ...
                "otp": str(row.get("otp_code")) if row.get("otp_code") else "",
                "otp_link": str(row.get("otp_link", "") or ""),
                "date": str(row.get("purchase_date", "") or "")
            })
...
def to_decimal(value):
    """Convert money values to exact Decimal without float precision artifacts."""
    try:
        if value is None:
            return Decimal("0")
...
def get_number_price():
    ...
    return to_decimal(doc["value"]) if doc else Decimal("0.10")
...
def set_number_price(new_price):
    ...
    settings_col.update_one(
        {"key": "number_price"},
        {"$set": {"value": format_money_exact(price)}},
        upsert=True
    )
...
def update_balance(user_id, amount_usd):
    ...
    users_col.update_one(
        ...
        {"$inc": {
            ...
            "total_recharge": Decimal128(amount)
        }}
    )
...
def deduct_balance(user_id, amount_usd):
    ...
    result = users_col.update_one(
        {"user_id": user_id, "balance": {"$gte": Decimal128(amount)}},
        {"$inc": {"balance": Decimal128(-amount)}}
    )
...
def delete_active_order(order_id):
    ...
    try:
        ...
        print(f"Order rollback error: {e}")
...
def get_google_sheet_sync_status():
    """Return unsynced purchase count for admin visibility."""
    try:
        return orders_col.count_documents({"google_sheet_synced": {"$ne": True}})
    except Exception as e:
        print(f"Google Sheet Sync Status Error: {e}")
        return -1
...
def get_admin_panel_data():
    ...
    sync_pending = get_google_sheet_sync_status()
    sync_str = "Not Set" if not get_google_sheet_url() else (f"⚠️ {sync_pending} Pending" if sync_pending > 0 else "✅ All Synced")
    status_str = "🟢 ONLINE" if bot_active else "🔴 OFF / MAINTENANCE"
    
    msg = (
        "<b>⚙️ ADMIN PANEL</b>\n\n"
        f"🤖 <b>Bot Status:</b> {status_str}\n"
        f"💰 <b>WhatsApp Price:</b> ${format_number_price(current_price)} USD\n"
        ...
        f"🔄 <b>Google Sheet Sync:</b> {sync_str}\n"
    ...
    markup = {
        "inline_keyboard": [
            ...
            [{"text": "🔄 Google Sheet Sync / Retry", "callback_data": "admin_sheet_sync", "style": "success"}],
...
def render_buyer_page(target_u_id, page=1, items_per_page=10):
    ...
    buyer_msg = (
        ...
        f"💰 <b>Current Balance:</b> ${format_money_exact(u_info[2])} USD\n"
        f"📊 <b>Total Recharge:</b> ${format_money_exact(u_info[3])} USD\n\n"
        f"📈 <b>BUYER STATS:</b>\n"
    ...
    try:
        ...
        if "message" in update:
            ...
            if state_data:
                ...
                if isinstance(state_data, str) and state_data == "BUY_MULTI_QTY":
                    try:
                        ...
                        total_cost = to_decimal(current_price) * Decimal(qty)
                        ...
                        try:
                            for _ in range(qty):
                                ...
                                if not phone:
                                    raise RuntimeError("Stock changed while processing the batch.")

                                order_id = save_active_order(user_id, phone, link)
                                purchased_list.append((phone, link, order_id))

                                if order_id is None:
                                    raise RuntimeError("Could not create purchase order.")

                        except Exception as e:
                            for p, l, oid in purchased_list:
                                add_stock_item(p, l)
                                delete_active_order(oid)
                            delete_user_state(user_id)
                            send_message(
                                chat_id,
                        ...
                        delivery_result = send_message(
                            chat_id,
                            res_msg,
                            reply_markup={"inline_keyboard": multi_btns}
                        )
                        ...
                        if not delivery_result.get("ok"):
                            ...
                            send_message(
                                chat_id,
                                "⚠️ <b>নম্বরগুলো পাঠাতে সমস্যা হয়েছে।</b> "
                                "আপনার ব্যালেন্স কাটা হয়নি। কিছুক্ষণ পর আবার চেষ্টা করুন।"
                            )
                            return
                        ...
                        if not deduct_balance(user_id, total_cost):
                            ...
                            send_message(
                                chat_id,
                                "⚠️ <b>পেমেন্ট প্রসেস করতে সমস্যা হয়েছে।</b> নম্বরগুলো স্টকে ফেরত দেওয়া হয়েছে এবং ব্যালেন্স কাটা হয়নি।"
                            )
                            return

                        # Google Sheet logging must never delay the user response.
                        now_for_log = datetime.now().strftime("%d-%b-%Y %I:%M %p")
                        for p, l, oid in purchased_list:
                            try:
                                order_doc = orders_col.find_one({"_id": oid}, {"purchase_date": 1})
                                p_date = (
                                    order_doc.get("purchase_date", now_for_log)
                                    if order_doc else now_for_log
                                )
                            except Exception:
                                p_date = now_for_log

                            threading.Thread(
                                target=background_log_purchase,
                                args=(user_id, username, p, l, p_date, oid),
                                daemon=True
                            ).start()
                        ...
                        send_message(
                            chat_id,
                            "❌ <b>ভুল ইনপুট!</b> কেবল পূর্ণসংখ্যা লিখুন (যেমন: 2, 5, 10)।"
                        )
                        return
               ...
               # Search User by Username
                ...
                # Admin Custom Deposit Input
                    ...
                    try:
                        usd_val = to_decimal(text.strip())
                        update_balance(target_user, usd_val)
                        send_message(chat_id, f"✅ <b>User ID {target_user}-কে ${format_money_exact(usd_val)} USD ব্যালেন্স যোগ করা হয়েছে।</b>")
                        send_message(target_user, f"🎉 <b>আপনার ডিপোজিট প্রসেস সফল হয়েছে! ${format_money_exact(usd_val)} USD অ্যাকাউন্টে যোগ করা হয়েছে।</b>")
                    except Exception:
                ...
                # Admin Refund Input
                    ...
                    try:
                        ...
                        send_message(chat_id, f"✅ <b>User @{target_name} (ID: {target_user})-কে ${format_money_exact(usd_val)} USD সফলভাবে রিফান্ড করা হয়েছে!</b>\n(রিফান্ড হিস্ট্রিতে রেকর্ডটি সেভ হয়েছে)", reply_markup=get_main_keyboard(is_admin))
                        send_message(target_user, f"🎉 <b>এডমিন আপনার অ্যাকাউন্টে ${format_money_exact(usd_val)} USD রিফান্ড যোগ করেছেন!</b>")
                    except Exception:
            ...
            if text == "/start":
                ...
                markup = {
                    "inline_keyboard": [
                        [{"text": f"👤 Buy Single Number (${format_number_price(current_price)})", "callback_data": "confirm_buy_usa", "style": "success"}],
                        [{"text": "🔢 Buy Multiple Numbers", "callback_data": "buy_multi_num", "style": "primary"}]
                    ]
                }
                send_message(chat_id, f"<b>WhatsApp Service Selected:</b>\n\nমূল্য: <b>${format_number_price(current_price)} USD / Number</b>", reply_markup=get_back_keyboard())
                ...
                prof_text = (
                    ...
                    f"🆔 <b>User ID:</b> <code>{user_id}</code>\n"
                    f"💰 <b>Current Balance:</b> ${format_money_exact(bal)} USD\n"
                    f"📊 <b>Total Recharge:</b> ${format_money_exact(tot)} USD\n"
            ...
            if data.startswith("buy_vpn_"):
                ...
                if to_decimal(bal) < to_decimal(pr):
                    edit_message(chat_id, message_id, f"❌ <b>পর্যাপ্ত ব্যালেন্স নেই!</b>\n{service.capitalize()}VPN কিনতে ${pr:.2f} USD লাগবে। আপনার ব্যালেন্স: ${format_money_exact(bal)} USD।")
                    return
            ...
            if data == "admin_toggle_bot_off" and is_admin:
                ...
                res_text = (
                    ...
                    f"🔗 <b>OTP Link:</b> {html.escape(str(link))}\n"
                ...
                delivery_result = send_message(chat_id, res_text, reply_markup=markup)
                if not delivery_result.get("ok"):
                    add_stock_item(phone, link)
                    ...
                    edit_message(
                        chat_id,
                        message_id,
                        "⚠️ <b>নম্বর পাঠাতে সমস্যা হয়েছে।</b> আপনার ব্যালেন্স কাটা হয়নি। কিছুক্ষণ পর আবার চেষ্টা করুন।"
                    )
                    return
                ...
                if not deduct_balance(user_id, current_price):
                    ...
                    edit_message(
                        chat_id,
                        message_id,
                        "⚠️ <b>পেমেন্ট প্রসেস করতে সমস্যা হয়েছে।</b> নম্বরটি আবার স্টকে ফেরত দেওয়া হয়েছে এবং ব্যালেন্স কাটা হয়নি।"
                    )
                    return

                try:
                    order_doc = orders_col.find_one({"_id": order_id}, {"purchase_date": 1})
                    p_date = (
                        order_doc.get("purchase_date", datetime.now().strftime("%d-%b-%Y %I:%M %p"))
                        if order_doc else datetime.now().strftime("%d-%b-%Y %I:%M %p")
                    )
                except Exception:
                    p_date = datetime.now().strftime("%d-%b-%Y %I:%M %p")

                # Never let Google Sheet/API latency block the buyer.
                threading.Thread(
                    target=background_log_purchase,
                    args=(user_id, username, phone, link, p_date, order_id),
                    daemon=True
                ).start()
                ...
                for u in users_list:
                    ...
                    display_title = f"👤 @{u_name} (${format_money_exact(u_bal)})" if u_name != "NoUsername" else f"👤 ID: {u_id} (${format_money_exact(u_bal)})"
            ...
            # PAGINATION CALLBACK FOR USER DETAILS PAGE NAVIGATION
            ...
            elif data == "admin_sheet_sync" and is_admin:
                if not get_google_sheet_url():
                    edit_message(chat_id, message_id, "❌ <b>Google Sheet URL সেট করা নেই।</b> আগে Set Google Sheet URL করুন।", reply_markup={"inline_keyboard": [[{"text": "⬅️ Back to Admin Panel", "callback_data": "admin_panel_back", "style": "danger"}]]})
                else:
                    pending_before = get_google_sheet_sync_status()
                    edit_message(chat_id, message_id, f"⏳ <b>Google Sheet Sync চলছে...</b>\nPending records: {pending_before}")
                    success, failed = sync_all_existing_orders_to_google_sheet()
                    pending_after = get_google_sheet_sync_status()
                    result_text = (
                        "🔄 <b>Google Sheet Sync সম্পন্ন!</b>\n\n"
                        f"✅ Saved/Confirmed: <b>{success}</b>\n"
                        f"❌ Failed: <b>{failed}</b>\n"
                        f"📌 এখনও Pending: <b>{max(pending_after, 0)}</b>"
                    )
                    edit_message(chat_id, message_id, result_text, reply_markup={"inline_keyboard": [
                        [{"text": "🔄 Retry Again", "callback_data": "admin_sheet_sync", "style": "success"}],
                        [{"text": "⬅️ Back to Admin Panel", "callback_data": "admin_panel_back", "style": "danger"}]
                    ]})
                ...
                send_message(chat_id, f"➕ <b>User @{target_uname} (ID: {target_u_id})-এর অ্যাকাউন্টে কত $ (USD) রিফান্ড করতে চান লিখে পাঠান:</b>\n\n<i>নোট: আপনি চাইলে রিফান্ড অ্যামাউন্টের সাথে স্পেস দিয়ে নম্বরের সংখ্যাও লিখতে পারেন (যেমন: <code>0.125 3</code>, <code>0.30 3</code> বা শুধু <code>0.125</code>)</i>", reply_markup=get_back_keyboard())
                ...
                send_message(chat_id, f"🏷️ <b>WhatsApp নম্বরের নতুন মূল্য ($ USD) লিখে পাঠান:</b>\n(বর্তমান রেট: ${format_number_price(current_price)} USD)", reply_markup=get_back_keyboard())
