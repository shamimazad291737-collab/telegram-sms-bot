...
def get_order_by_phone(phone):
    conn = sqlite3.connect('bot_database.db')
    cursor = conn.cursor()
    # '+' সহ এবং '+' ছাড়া দুটি ফরম্যাটই চেক করা হবে
...
def handle_update(update):
    ...
    # CALLBACK QUERY HANDLING
        ...
        if data == "confirm_buy_usa":
            ...
            # callback_data তে '+' ছাড়া বিশুদ্ধ ডিজিট দেওয়া হচ্ছে
            clean_phone = phone.replace("+", "")
            markup = {
                "inline_keyboard": [
                    [{"text": "🔄 Check OTP", "callback_data": f"chk_otp_{clean_phone}"}],
        ...
        # FIXED REGEX OTP EXTRACTION
            ...
            if order:
                ...
                try:
                    ...
                    # HTML Tag সরিয়ে কেবল প্লেন টেক্সট বের করা
                    clean_text = re.sub(r'<[^>]+>', ' ', raw_html)

                    # প্রথমে ৬ ডিজিটের কোড খোঁজা
                    otp_match = re.search(r'\b(\d{6})\b', clean_text)
                    
                    # যদি না পাওয়া যায়, তবে ৩ ডিজিট-৩ ডিজিট (যেমন: 365-589) ফরম্যাট খোঁজা
                    if not otp_match:
